...
export const EXPLANATION = "Optional per-chat executor. Main / Thinker delegates bounded tasks to the Executor and reviews the results. Off uses the normal Main / Thinker flow. Main / Thinker is the model selected alongside. This setting does not change the Main model or global defaults.";
const ENDPOINT = "/api/executor.model";
const PLUGIN_ID = "dsh-executor-model";
const EXECUTOR_LABEL = /^Executor · ([^·]+?)(?: · |$)/;

export function catalogModelName(groups, selection) {
...
export function executorCallInfo(block) {
	let args;
	try { args = JSON.parse(block?.arguments ?? block?.argsRaw ?? block?.call?.argsRaw ?? "{}"); } catch { args = {}; }
...
/** Keep provider-owned ids opaque, including slashes and punctuation. */
...
export function normalizeSelection(value) {
	...
	if (typeof value !== "object" || value === null || typeof value.provider !== "string" || value.provider.length === 0 || typeof value.model !== "string" || value.model.length === 0 || (value.reasoningEffort !== undefined && (typeof value.reasoningEffort !== "string" || value.reasoningEffort.length === 0))) {
		throw new Error("The host returned an invalid executor selection.");
	}
	return Object.freeze({ provider: value.provider, model: value.model, ...(value.reasoningEffort === undefined ? {} : { reasoningEffort: value.reasoningEffort }) });
}

/** Select only effort ids advertised by this exact model, never a Main setting. */
export function lowestAdvertisedEffort(model) {
...
export function selectionForChoice(choice, current = null) {
	...
	return normalizeSelection({ provider: choice.group.id, model: choice.model.id, ...(reasoningEffort === undefined ? {} : { reasoningEffort }) });
}

/**
 * A per-session observable. GETs never overwrite a newer write. Writes
 * serialize on the wire (not just in React) so rapid changes persist in order.
 * Host storage is authoritative; focus/open reads pick up other browser tabs.
 */
export class ExecutorSelectionController {
	...
	async request(method, selection) {
		...
		try {
			...
			// Never echo arbitrary error response bodies (which may contain credentials).
			...
			catch { throw new Error("The host returned an invalid executor settings response."); }
			return normalizeSelection(body?.selection);
		} finally { this.requests.delete(aborter); }
	}
	...
	save(value) {
		let selection;
		try { selection = normalizeSelection(value); }
		...
		++this.clock; // Invalidate GETs started before this user choice.
		++this.pendingWrites;
	...
	resetConnected() {
		++this.clock;
		// A response already in flight belongs to the previous connection generation.
...
/** Focus/open refresh is cross-tab synchronization without browser-only persistence. */
export function installRefreshListeners(refresh, browserWindow = window, pageDocument = document) {
	const visible = () => { if (pageDocument.visibilityState === "visible") refresh(); };
	browserWindow.addEventListener("focus", refresh);
	pageDocument.addEventListener("visibilitychange", visible);
	return () => { browserWindow.removeEventListener("focus", refresh); pageDocument.removeEventListener("visibilitychange", visible); };
}
...
export function ExecutorModelControls({ controller, directory, available }) {
	...
	const missing = saved.selection !== null && chosen === undefined && catalog.status !== "loading" && catalog.status !== "idle";
	const efforts = chosen?.model.reasoning?.efforts ?? [];
	const effort = saved.selection?.reasoningEffort ?? "";
	const missingEffort = chosen !== undefined && effort !== "" && !efforts.some(item => item.id === effort);
	...
	const setSelection = selection => { controller.save(selection).catch(() => {}); };
	return React.createElement("div", { className: "em-control", title: EXPLANATION, "data-executor-model": controller.sessionId },
		React.createElement("label", { className: "em-label" }, "Executor",
			React.createElement("select", {
				className: "em-select", "aria-label": "Executor model", "aria-describedby": `${explanationId} ${statusId}`, "aria-busy": busy || loading, value: key, disabled,
				onFocus: retry,
				onPointerDown: retry,
				onChange: event => {
					if (event.target.value === "") { setSelection(null); return; }
					const choice = choices.find(item => item.key === event.target.value);
					if (choice !== undefined) setSelection(selectionForChoice(choice, saved.selection));
				},
			},
			...
			!missing && saved.selection !== null && chosen === undefined && React.createElement("option", { value: key }, saved.selection.model),
			...catalog.groups.map(group => React.createElement("optgroup", { key: group.id, label: group.name },
		...
		(efforts.length > 0 || missingEffort) && React.createElement("label", { className: "em-label em-effort-label" }, "Effort",
			React.createElement("select", {
				className: "em-select em-effort", "aria-label": "Executor reasoning effort", "aria-describedby": explanationId, value: effort, disabled,
				onChange: event => {
		...
		React.createElement("span", { id: statusId, className: message && (saved.error || catalogError || missing || missingEffort || !available || catalog.failures.length > 0) ? "em-status em-warning" : "em-sr-only", role: saved.error ? "alert" : "status", "aria-live": "polite" }, message,
...
/** Key local React state by chat; stores never cross session boundaries. */
export function ExecutorModelSelect(props) {
	return React.createElement(ExecutorModelControls, { ...props, key: props.controller.sessionId });
}

const IDLE_SNAPSHOT = Object.freeze({ current: null, groups: [] });
const IDLE_DIRECTORY = { getSnapshot: () => IDLE_SNAPSHOT, subscribe: () => () => {} };

export function CallerChip({ sessionId, useSessions = () => undefined, directory, address }) {
	const catalog = useStore(directory?.store ?? IDLE_DIRECTORY);
	React.useEffect(() => { directory?.load?.().catch(() => {}); }, [directory]);
	...
	const role = address !== undefined && address !== null ? "Executor" : "Main";
	const model = executor?.model || (role === "Main" ? mainName : "");
	if (model === "" && role === "Executor" && (entry?.label === undefined || entry.label === "")) return null;
	const label = model === "" ? role : `${role} · ${model}`;
...
export function ExecutorToolCard({ block, toolName = "executor" }) {
	const info = executorCallInfo(block);
	const model = info.model ? `Executor · ${info.model}` : "Executor";
	return React.createElement("div", { className: "em-tool", "data-executor-tool": toolName, title: EXPLANATION },
...
export function apply(ctx) {
	...
	ctx.effect(() => () => { for (const controller of controllers.values()) controller.dispose(); controllers.clear(); }, "executor-model: session stores");
	ctx.on("connection/reset", () => { for (const controller of controllers.values()) controller.resetConnected().catch(() => {}); });
	ctx.slots.inject("conversation.session.header.actions", () => ctx.slots.register({
		...
		inject: sessionId => {
			try {
				...
				return {
					directory: address === undefined ? ctx.modelDirectories.directoryFor(sessionId) : undefined,
					address,
				};
			} catch {
				return { directory: undefined, address: undefined };
			}
		},
	}, CallerChip));
	...
	ctx.slots.inject("conversation.input.right", () => ctx.slots.register({
		...
		inject: sessionId => {
			...
			if (controller === undefined) {
				...
				if (scope === undefined || binding === undefined) throw new Error("Executor selector requires an attached chat.");
				controller = new ExecutorSelectionController(sessionId, { available });
				controllers.set(sessionId, controller);
				const owned = controller;
				scope.effect(() => () => { owned.dispose(); if (controllers.get(sessionId) === owned) controllers.delete(sessionId); }, "executor-model: chat scope");
			}
